...
<div>
	...
	<popup v-model="show" height="100%"> 
	  <div v-if="isApiClound"  style="width: 100%;height: 25px;background: black;"> 
     </div>
		<x-header :left-options="{showBack: false}">{{dialog.name}}</x-header>
		 
		<div>
			<ab-cust-dialog-show-list   v-if="dialog.style=='list'" ref="showLIst" :isCheckbox="isCheckbox"     :dynamic="dynamic" :isdyKey="isdyKey" :dialog="dialog" :initData="initData"> </ab-cust-dialog-show-list>
 ...
 */
export default {
	props:["dialogKey","dialogSetting","model","isCheckbox","dynamicKey","dynamic","isdyKey"],//dialogKey:列表对话框的key;dialogSetting:对话框的配置修改
	data: function () {
	  return {
		  show:false,
	...
	watch: {
		dynamicKey(value){  
			 this.$refs.showLIst.emptyRows(value);
	...
	methods: {
    	...
    	var post = Vue.baseService.postForm(url,{
			key : this.dialogKey,
			'pxjb^VEQ':this.dynamic?this.dynamicKey:undefined
		});
		...
		getData:function(){
			...
			this.show = false;//关闭popup
				if(this.$parent.$parent.setData){
				this.$parent.$parent.setData(data)
			} 
		},
		//处理返回数据
		handleData:function(data){ 
			if(!this.model){
				this.model=this.isCheckbox?[]:{}
			} 
			var valueMap = this.valueMap;    
			if(Array.isArray(this.model)){//model是列表
				var list = this.model;
				var arr = [];
				//  复选框为空时 清除 绑定的vmodel关联数据 
				//   if(data.length===0){
				...
				//   }
					...
					//  list.splice(0,list.length,...arr) 
				      //注  复选框状态 model 需要添加 .sync 修饰符
					   this.$emit('update:model',arr)
					 
					//  this.$set(this.model,0,arr);
			//    this.model=list;
			}else{//组件是对象
				for(var key in valueMap){
					var val = valueMap[key];
