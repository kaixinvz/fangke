<template>
  <div>
    <!-- 按钮 -->
    <span v-on:click="showDialog" v-if="permission=='w'||permission=='b'">
		<slot></slot>
	</span>
    <a v-if="permission=='w'||permission=='r'||permission=='b'" style="margin-left: 5px;" v-for="file in files" :href="file.href" class="weui-btn weui-btn_mini weui-btn_primary fa fa-download">
      {{file.name}}
    </a>
    <!-- 附件对话框的内容 -->
