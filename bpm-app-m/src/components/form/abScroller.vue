...
<template>
		...
		<script>
		import Vue from 'vue';
		import SearchBg from '@/assets/img/f7c014380bc542d5c00e36eac59d601.jpg'
  ...
  import moment from 'moment'
		...
		export default {
			...
			//marginTop:由于scroller组件的bug，其无法计算出离顶部的距离，目前组件会默认进行x-header的50px
			 ...
			 // ,"pageSize"
			props:["url","height","list","marginTop","conditions"],
			data: function () {
			  return {
				rows :[],//列表数据
				  pageData :{},//当前页数据
			...
			computed:{
				...mapState(["isApiClound"]),
				 scrollStyle(){
					   return this.isApiClound?"margin-top:30px;":""
				 }
			},
			methods: {
				//上拉刷新
				...
				search(offset,limit,callback){
					...
					post.then(function(data){ 
						...
						data.rows.forEach(function(item,index){
							if(item.data&&item.data.pcrw){
								const pcrw=item.data.pcrw;
								const lczt=pcrw.lczt;
								let   ztValue=""
								if(lczt>=1&&lczt<=4){
									ztValue="审批阶段"
								}else if(lczt>=5&&lczt<=7){
									ztValue="整改阶段"
								}else if(lczt>=8&&lczt<=10){
									ztValue="复查阶段"
								}else if(lczt==12){
									 ztValue="驳回阶段"
								}
							     
								item['title']=moment(pcrw.pcsj).format('YYYY-MM-DD')+":"+ztValue+"-"+pcrw.zgqdList[0].yhsm||""; 
							}else if(item.data&&item.data.aqpxjh){
						...
						const  pageSize=that.pageSize||10;
